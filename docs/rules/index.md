...
- [`git.md`](git) — git workflow rules: Solo Mode, GitHub Flow, branch naming, Auto-Commit, Conventional Commits, code quality gates, deployment triggers, and environment branch promotion. Activate features by declaring them in your project's `CLAUDE.md` (`git-strategy: solo`, `git-auto-commit: true`, `git-merge-strategy: squash`).
